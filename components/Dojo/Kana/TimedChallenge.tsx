...
import { Timer, Target, TrendingUp, RotateCcw, Play, ArrowLeft, CheckCircle2, XCircle, X } from 'lucide-react';
...
export default function TimedChallengeKana() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('timedChallengeDuration', challengeDuration.toString());
    }
  }, [challengeDuration]);
    ...
    return (
      <div className="min-h-[100dvh] flex flex-col lg:flex-row items-start justify-center p-4 gap-6">
        <div className="max-w-md w-full lg:max-w-lg text-center space-y-6">
          ...
          <h1 className="text-2xl font-bold text-[var(--secondary-color)]">Timed Challenge</h1>
          <p className="text-[var(--muted-color)]">
            Test your kana recognition speed! Answer as many questions as possible before time runs out.
          </p>
          ...
          <div className="bg-[var(--card-color)] rounded-lg p-4 space-y-3">
            <p className="text-sm font-medium text-[var(--secondary-color)]">Challenge Duration:</p>
  ...
  if (isFinished) {
    ...
    const accuracy = totalQuestions > 0 ? Math.round((timedCorrectAnswers / totalQuestions) * 100) : 0;
    const questionsPerMinute = totalQuestions > 0 ? ((totalQuestions / challengeDuration) * 60).toFixed(1) : '0';
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div className="bg-[var(--bg-color)] max-w-2xl w-full max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl relative">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-3xl font-bold text-[var(--secondary-color)]">Challenge Complete!</h1>
            <p className="text-[var(--muted-color)]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[var(--card-color)] rounded-lg p-4 space-y-2 border border-[var(--border-color)]">
              ...
              <p className="text-2xl font-bold text-[var(--secondary-color)]">🔥 {timedStreak}</p>
            </div>
            
            <div className="bg-[var(--card-color)] rounded-lg p-4 space-y-2 border border-[var(--border-color)]">
              <p className="text-sm text-[var(--muted-color)]">Total Answers</p>
              <p className="text-2xl font-bold text-[var(--secondary-color)]">{totalQuestions}</p>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {reachedGoals.map((goal) => (
                      ...
                      >
                        <span className="text-[var(--secondary-color)]">{goal.label}</span>
      ...
      <div className="max-w-md w-full lg:max-w-lg space-y-6">
        ...
        <div className="w-full bg-[var(--border-color)] rounded-full h-2">
          <div 
            ...
            style={{ width: `${((challengeDuration - timeLeft) / challengeDuration) * 100}%` }}
          />
        </div>
          ...
          {goalTimers.nextGoal && (
            <div className={clsx(
              ...
              <p className="text-xs text-[var(--secondary-color)] mt-1 text-center">
                ...
                {(goalTimers.nextGoal.targetSeconds % 60).toString().padStart(2, '0')}
              </p>
            </div>
          )}
        </div>
